...
FUNFAIRSALESOL=`grep ^FUNFAIRSALESOL= settings.txt | sed "s/^.*=//"`
FUNFAIRSALETEMPSOL=`grep ^FUNFAIRSALETEMPSOL= settings.txt | sed "s/^.*=//"`
FUNFAIRSALEJS=`grep ^FUNFAIRSALEJS= settings.txt | sed "s/^.*=//"`
TOKENCONTROLLERLEDGERSOL=`grep ^TOKENCONTROLLERLEDGERSOL= settings.txt | sed "s/^.*=//"`
TOKENCONTROLLERLEDGERTEMPSOL=`grep ^TOKENCONTROLLERLEDGERTEMPSOL= settings.txt | sed "s/^.*=//"`
TOKENCONTROLLERLEDGERJS=`grep ^TOKENCONTROLLERLEDGERJS= settings.txt | sed "s/^.*=//"`
...
printf "FUNFAIRSALESOL               = '$FUNFAIRSALESOL'\n"
printf "FUNFAIRSALETEMPSOL           = '$FUNFAIRSALETEMPSOL'\n"
printf "FUNFAIRSALEJS                = '$FUNFAIRSALEJS'\n"
printf "TOKENCONTROLLERLEDGERSOL     = '$TOKENCONTROLLERLEDGERSOL'\n"
printf "TOKENCONTROLLERLEDGERTEMPSOL = '$TOKENCONTROLLERLEDGERTEMPSOL'\n"
printf "TOKENCONTROLLERLEDGERJS      = '$TOKENCONTROLLERLEDGERJS'\n"
...
`cp $FUNFAIRSALESOL $FUNFAIRSALETEMPSOL`
`cp $TOKENCONTROLLERLEDGERSOL $TOKENCONTROLLERLEDGERTEMPSOL`
...
DIFFS1=`diff $FUNFAIRSALESOL $FUNFAIRSALETEMPSOL`
echo "--- Differences $FUNFAIRSALETEMPSOL ---"
echo "$DIFFS1"

DIFFS2=`diff $TOKENCONTROLLERLEDGERSOL $TOKENCONTROLLERLEDGERTEMPSOL`
echo "--- Differences $TOKENCONTROLLERLEDGERTEMPSOL ---"
echo "$DIFFS2"

echo "var ffsOutput=`solc --optimize --combined-json abi,bin,interface $FUNFAIRSALETEMPSOL`;" > $FUNFAIRSALEJS
echo "var tclOutput=`solc --optimize --combined-json abi,bin,interface $TOKENCONTROLLERLEDGERTEMPSOL`;" > $TOKENCONTROLLERLEDGERJS

geth --verbosity 3 attach $GETHATTACHPOINT << EOF | tee $TEST1OUTPUT
loadScript("$FUNFAIRSALEJS");
loadScript("$TOKENCONTROLLERLEDGERJS");
loadScript("functions.js");

var ffsAbi = JSON.parse(ffsOutput.contracts["$FUNFAIRSALETEMPSOL:FunFairSale"].abi);
var ffsBin = "0x" + ffsOutput.contracts["$FUNFAIRSALETEMPSOL:FunFairSale"].bin;
var tokenAbi = JSON.parse(tclOutput.contracts["$TOKENCONTROLLERLEDGERTEMPSOL:Token"].abi);
var tokenBin = "0x" + tclOutput.contracts["$TOKENCONTROLLERLEDGERTEMPSOL:Token"].bin;
var controllerAbi = JSON.parse(tclOutput.contracts["$TOKENCONTROLLERLEDGERTEMPSOL:Controller"].abi);
var controllerBin = "0x" + tclOutput.contracts["$TOKENCONTROLLERLEDGERTEMPSOL:Controller"].bin;
var ledgerAbi = JSON.parse(tclOutput.contracts["$TOKENCONTROLLERLEDGERTEMPSOL:Ledger"].abi);
var ledgerBin = "0x" + tclOutput.contracts["$TOKENCONTROLLERLEDGERTEMPSOL:Ledger"].bin;


console.log("DATA: ffsAbi=" + JSON.stringify(ffsAbi));
...
// -----------------------------------------------------------------------------
var ffsMessage = "Deploy FunFairSale Contract";
console.log("RESULT: " + ffsMessage);
var ffsContract = web3.eth.contract(ffsAbi);
console.log(JSON.stringify(ffsContract));
var ffsTx = null;
var ffsAddress = null;
var ffs = ffsContract.new(contractOwnerAccount, {from: contractOwnerAccount, data: ffsBin, gas: 6000000},
  function(e, contract) {
    if (!e) {
      if (!contract.address) {
        ffsTx = contract.transactionHash;
      } else {
        ffsAddress = contract.address;
        addAccount(ffsAddress, "FunFairSale");
        addFfsContractAddressAndAbi(ffsAddress, ffsAbi);
        console.log("DATA: ffsAddress=" + ffsAddress);
      }
    }
  }
);
...
printTxData("ffsAddress=" + ffsAddress, ffsTx);
printBalances();
failIfGasEqualsGasUsed(ffsTx, ffsMessage);
